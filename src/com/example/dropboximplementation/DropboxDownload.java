...
public class DropboxDownload extends Activity implements OnItemClickListener {
	...
	private Handler mHandler = new Handler() {
		public void handleMessage(android.os.Message msg) {
			if (msg.what == 0) {
				lvDropboxDownloadFilesList.setAdapter(new DropboxDownloadFileAdapter(
				...
				// Intent intent=new Intent();
	             //   intent.putExtra("path_of_dropbox_downloadedFile", ""+msg.obj.toString());
	...
	private void copy(final Entry fileSelected, final File localFile) {
		final ProgressDialog pd = ProgressDialog.show(DropboxDownload.this,
				"Attachment is going on...", "Please wait...");
			...
			public void run() {
				...
				try {
					...
					while (true)
					{
						...
						if (read <= 0) // download finished so break continue the loop as well as thread
						{
							// as download is completed so dismiss the progressbar and send a message to handler 
							pd.dismiss();
						...
						else if(pd.isShowing()==false)// break  the loop as well as thread if user press the back button that means close the progressbar
						{
							// if the user press back button then the progressbar will be dismissed;
							//but my program creates the file in the directory but can't write the content 
							// so i need to delete the file from the directory as i dont want to download the file 
							if(localFile.exists())
							{
								localFile.delete();
							}
						...
						else // continue the loop as well as thread
						{
